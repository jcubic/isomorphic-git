...
export async function push ({
  ...
  if (!line.startsWith('unpack ')) {
    throw new Error(
      `push.js:108 E99 Unparsable response from server! Expected 'unpack ok' or 'unpack [error message]' but got '${line}'`
    )
  }
  if (line === 'unpack ok') {
    result.ok = ['unpack']
...
export async function listCommits ({
  ...
  async function walk (oid) {
    ...
    if (type !== 'commit') {
      throw new Error(`push.js:160 E100 Expected type commit but type is ${type}`)
    }
...
export async function pack ({
  ...
  function writeObject ({ stype, object }) {
    ...
    if (type === undefined) throw new Error('push.js:233 ' + 'E101 ' + 'Unrecognized type: ' + stype)
