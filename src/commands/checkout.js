...
export async function checkout ({
  ...
  if (ref === undefined) {
    throw new Error('checkout.js:22 E80 Cannot checkout ref "undefined"')
  }
  // Get tree oid
  let oid
  try {
  ...
  try {
    ...
    throw new Error(
      `checkout.js:60 E81 Failed to checkout ref '${ref}' because commit ${oid} is not available locally. Do a git fetch to make the branch available locally.`
    )
  }
  if (commit.type !== 'commit') {
    throw new Error(`checkout.js:64 E82 Unexpected type: ${commit.type}`)
  }
  ...
  if (type !== 'tree') throw new Error(`checkout.js:70 E83 Unexpected type: ${type}`)
...
async function writeTreeToDisk ({ fs: _fs, dir, gitdir, index, prefix, tree }) {
  ...
  for (let entry of tree) {
    ...
    switch (type) {
      ...
      default:
        throw new Error(
          `checkout.js:127 E84 Unexpected object type ${type} found in tree for '${entrypath}'`
        )
    }
  }
}
