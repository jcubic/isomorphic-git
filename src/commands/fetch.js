...
async function fetchPackfile ({
  ...
  if (depth !== null) {
    if (Number.isNaN(parseInt(depth))) {
      throw new Error(`fetch.js:109 E86 Invalid value for depth argument: ${depth}`)
    }
  ...
  if (depth !== null && !remoteHTTP.capabilities.has('shallow')) {
    throw new Error(`fetch.js:136 E87 Remote does not support shallow fetches`)
  }
  if (since !== null && !remoteHTTP.capabilities.has('deepen-since')) {
    throw new Error(`fetch.js:139 E88 Remote does not support shallow fetches by date`)
  }
  if (exclude.length > 0 && !remoteHTTP.capabilities.has('deepen-not')) {
    throw new Error(
      `fetch.js:143 E89 Remote does not support shallow fetches excluding commits reachable by refs`
    )
  }
  if (relative === true && !remoteHTTP.capabilities.has('deepen-relative')) {
    throw new Error(
      `fetch.js:148 E90 Remote does not support shallow fetches relative to the current shallow depth`
    )
  }
  ...
  response.packetlines.pipe(
    through2(async (data, enc, next) => {
      ...
      if (line.startsWith('shallow')) {
        ...
        if (oid.length !== 40) {
          throw new Error(`fetch.js:225 E91 non-40 character 'shallow' oid: ${oid}`)
        }
        oids.add(oid)
        ...
        let oid = line.slice(-41).trim()
        if (oid.length !== 40) {
          throw new Error(`fetch.js:232 E92 non-40 character 'shallow' oid: ${oid}`)
        }
        oids.delete(oid)
        await GitShallowManager.write({ fs, gitdir, oids })
      }
