  ...
  static async updateRemoteRefs ({
    ...
    for (let value of refs.values()) {
      if (!value.match(/[0-9a-f]{40}/)) {
        throw new Error(`GitRefManager.js:32 E109 Unexpected ref contents: '${value}'`)
      }
    }
    ...
    if (!refspecs) {
      ...
      if (refspecs.length === 0) {
        throw new Error(
          `GitRefManager.js:40 Could not find a fetch refspec fot remote '${remote}'.
...
fetch = +refs/heads/*:refs/remotes/origin/*`
  ...
  static async writeRef ({ fs: _fs, gitdir, ref, value }) {
    ...
    // Validate input
    if (!value.match(/[0-9a-f]{40}/)) {
      throw new Error(`GitRefManager.js:102 E110 Unexpected ref contents: '${value}'`)
    }
    const normalizeValue = value => value.trim() + '\n'
  ...
  static async resolve ({ fs: _fs, gitdir, ref, depth }) {
    ...
    // Do we give up?
    throw new Error(`GitRefManager.js:138 E111 Could not resolve reference ${ref}`)
  }
  static async expand ({ fs: _fs, gitdir, ref }) {
    ...
    // Do we give up?
    throw new Error(`GitRefManager.js:155 E112 Could not expand ref ${ref}`)
  }
  static resolveAgainstMap ({ ref, depth, map }) {
    ...
    // Do we give up?
    throw new Error(`GitRefManager.js:182 E113 Could not resolve reference ${ref}`)
  }
