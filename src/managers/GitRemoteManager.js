...
export class GitRemoteManager {
  static getRemoteHelperFor ({ url }) {
    ...
    if (!parts) {
      throw new Error(`GitRemoteManager.js:42 E119 Cannot determine protocol of remote URL: "${url}"`)
    }
    if (remoteHelpers.has(parts.transport)) {
      return remoteHelpers.get(parts.transport)
    }
    throw new Error(
      `GitRemoteManager.js:48 E120 Git remote "${url}" uses an unrecognized transport protocol: "${
        parts.transport
      }"`
    )
  }
