...
export class GitRemoteConnection {
  static async discover (service, res) {
    ...
    if (lineOne === true) throw new Error('GitRemoteConnection.js:20 E114 Bad response from git server.')
    if (lineOne.toString('utf8') !== `# service=${service}\n`) {
      throw new Error(
        `GitRemoteConnection.js:23 E115 Expected '# service=${service}\\n' but got '${lineOne.toString(
          'utf8'
        )}'`
      )
    }
    ...
    const nextBit = async function () {
      ...
      switch (line[0]) {
        ...
        case 3: // fatal error message just before stream aborts
          ...
          packfile.destroy(new Error('GitRemoteConnection.js:93 ' + 'E116 ' + error.toString('utf8')))
          return
        default:
