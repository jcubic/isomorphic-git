  ...
  static async read ({ fs: _fs, gitdir, oid, format = 'content' }) {
    ...
    if (!file) {
      ...
      if (text !== null && text.includes(oid)) {
        throw new Error(
          `GitObjectManager.js:64 E107 Failed to read git object with oid ${oid} because it is a shallow commit`
        )
      }
    }
    // Finally
    if (!file) {
      throw new Error(`GitObjectManager.js:70 E108 Failed to read git object with oid ${oid}`)
    }
    if (format === 'deflated') {
