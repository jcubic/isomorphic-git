...
export class GitRemoteHTTP {
  ...
  static async discover ({ service, url, auth }) {
    ...
    if (res.statusCode !== 200) {
      throw new Error(`GitRemoteHTTP.js:32 E117 HTTP Error: ${res.statusCode} ${res.statusMessage}`)
    }
  ...
  static async connect ({ service, url, auth, stream }) {
    ...
    if (res.statusCode !== 200) {
      throw new Error(`GitRemoteHTTP.js:53 E118 HTTP Error: ${res.statusCode} ${res.statusMessage}`)
    }
