...
export class GitObject {
  ...
  static unwrap ({ oid, buffer }) {
    if (oid) {
      ...
      if (sha !== oid) {
        throw new Error(`GitObject.js:27 E131 SHA check failed! Expected ${oid}, computed ${sha}`)
      }
    }
    ...
    if (parseInt(length) !== actualLength) {
      throw new Error(
        `GitObject.js:38 E132 Length mismatch: expected ${length} bytes but got ${actualLength} instead.`
      )
    }
    return {
      type,
