...
function parseBuffer (buffer) {
  ...
  while (cursor < buffer.length) {
    ...
    if (space === -1) {
      throw new Error(
        `GitTree.js:17 E141 GitTree: Error parsing buffer at byte location ${cursor}: Could not find the next space character.`
      )
    }
    let nullchar = buffer.indexOf(0, cursor)
    if (nullchar === -1) {
      throw new Error(
        `GitTree.js:23 E142 GitTree: Error parsing buffer at byte location ${cursor}: Could not find the next null character.`
      )
    }
...
function limitModeToAllowed (mode) {
  ...
  throw new Error(`GitTree.js:47 E143 Could not understand file mode: ${mode}`)
}
  ...
  */
  constructor (entries) {
    if (Buffer.isBuffer(entries)) {
      ...
      throw new Error('GitTree.js:71 E144 invalid type passed to GitTree constructor')
    }
  }
  static from (tree) {
