...
export class GitPackIndex {
  ...
  static async fromIdx ({ idx, getExternalRefDelta }) {
    ...
    if (version !== 2) {
      throw new Error(
        `GitPackIndex.js:68 E133 Unable to read version ${version} packfile IDX. (Only version 2 supported)`
      )
    }
    // Verify checksums
    ...
    if (shaClaimed !== shaComputed) {
      throw new Error(
        `GitPackIndex.js:76 E134 Invalid checksum in IDX buffer: expected ${shaClaimed} but saw ${shaComputed}`
      )
    }
    if (idx.byteLength > 2048 * 1024 * 1024) {
      throw new Error(
        `GitPackIndex.js:81 E135 To keep implementation simple, I haven't implemented the layer 5 feature needed to support packfiles > 2GB in size.`
      )
    }
    let fanout = []
    for (let i = 0; i < 256; i++) {
  ...
  async read ({ oid }) {
    if (!this.offsets[oid]) {
      if (this.getExternalRefDelta) {
        ...
        throw new Error(`GitPackIndex.js:368 E136 Could not read object ${oid} from packfile`)
      }
    }
  ...
  async readSlice ({ start }) {
    ...
    if (!this.pack) {
      throw new Error(
        'GitPackIndex.js:387 E137 Tried to read from a GitPackIndex with no packfile loaded into memory'
      )
    }
    ...
    if (type === undefined) {
      throw new Error('GitPackIndex.js:397 ' + 'E138 ' + 'Unrecognized type: 0b' + btype.toString(2))
    }
    ...
    if (object.byteLength !== length) {
      throw new Error(
        `GitPackIndex.js:427 E139 Packfile told us object would have length ${length} but it had length ${
          object.byteLength
        }`
      )
    }
