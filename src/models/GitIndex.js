...
function parseBuffer (buffer) {
  ...
  if (shaClaimed !== shaComputed) {
    throw new Error(
      `GitIndex.js:58 E125 Invalid checksum in GitIndex buffer: expected ${shaClaimed} but saw ${shaComputed}`
    )
  }
  ...
  if (magic !== 'DIRC') {
    throw new Error(`GitIndex.js:65 E126 Inavlid dircache magic file number: ${magic}`)
  }
  let version = reader.readUInt32BE()
  if (version !== 2) throw new Error(`GitIndex.js:68 E127 Unsupported dircache version: ${version}`)
  ...
  while (!reader.eof() && i < numEntries) {
    ...
    if (pathlength < 1) throw new Error(`GitIndex.js:92 E128 Got a path length of: ${pathlength}`)
    entry.path = reader.toString('utf8', pathlength)
    // The next bit is awkward. We expect 1 to 8 null characters
    let tmp = reader.readUInt8()
    if (tmp !== 0) {
      throw new Error(`GitIndex.js:97 E129 Expected 1-8 null characters but got '${tmp}'`)
    }
    let numnull = 1
...
export class GitIndex {
  ...
  constructor (index) {
    ...
    if (Buffer.isBuffer(index)) {
      ...
      throw new Error('GitIndex.js:121 E130 invalid type passed to GitIndex constructor')
    }
  }
  static from (buffer) {
